...
def load_StereoImages(dirpath='/home/martin/Documents/Study/Deeplearning/Depth-Estimation-with-CNN/depth-stereo/depth-estimation/'):
    # load training data
    X , X_left,X_right, y =[], [], [], []
    n = 0
    ...
    for n in range(1,1000):
        ...
        with open(path, 'rb') as f:
            ...
            #print("kukuku : {}".format(size_StereoIm))
            #print("kukuku : {}".format(size_StereoIm[1]/2))
            ...
            Im_right = StereoIm[0,...,int(size_StereoIm[1]/2):int(size_StereoIm[1])]
#             StereoIm = StereoIm.reshape(-1,1)
        X.append(StereoIm)
        X_left.append(Im_left)
        X_right.append(Im_right)
        
    X = np.concatenate((X_left,X_right),1).reshape(-1, 2, int(size_StereoIm[0]), int(size_StereoIm[1]/2)).astype(np.float32)
    X_left = np.concatenate(X_left).reshape(-1, 1, int(size_StereoIm[0]), int(size_StereoIm[1]/2)).astype(np.float32)
    X_right = np.concatenate(X_right).reshape(-1, 1, int(size_StereoIm[0]), int(size_StereoIm[1]/2)).astype(np.float32)
